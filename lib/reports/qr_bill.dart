...
class QRBill {
  ...
  QRBill.fromJson(Map<String, dynamic> json) {
    setQrType(json['qr_type']);
    setVersion(json['version']);
    setCodingType(json['coding_type']);
    setIBAN(json['iban']);
    setAmount(json['amount']);
    setCurrency(json['currency']);
    setReference(json['reference_type'], json['reference']);
    setAdditionalInfo(json['additional_info']);
    Actor creditor = Actor.fromJson(json['creditor']);
    setActor(
        typeId: actorCR,
        name: creditor.name,
        ...
        postalcode: creditor.postcode,
        location: creditor.location,
        country: creditor.country);
    Actor ultimateCreditor = Actor.fromJson(json['ultimate_creditor']);
    setActor(
        typeId: actorUCR,
        name: ultimateCreditor.name,
        addressType: ultimateCreditor.addressType,
        address1: ultimateCreditor.address1,
        address2: ultimateCreditor.address2,
        postalcode: ultimateCreditor.postcode,
        location: ultimateCreditor.location,
        country: ultimateCreditor.country);
    Actor ultimateDebitor = Actor.fromJson(json['ultimate_debitor']);
    setActor(
        typeId: actorUDR,
        name: ultimateDebitor.name,
        addressType: ultimateDebitor.addressType,
        address1: ultimateDebitor.address1,
        address2: ultimateDebitor.address2,
        postalcode: ultimateDebitor.postcode,
        location: ultimateDebitor.location,
        country: ultimateDebitor.country);
    setBillInfo(json['bill_information']);
    setTrailer(json['trailer']);
    List<int> dueDate = json['due_date'];
    if (dueDate.isNotEmpty) {
      setDueDate(dueDate[0], dueDate[1], dueDate[2]);
    }
    setAlternativeSchema(schemas: json['alternative_schema']);
  }

  Map<String, dynamic> toJson() {
    String jsonString(String? input) => RegExp.escape(input ?? '');
    return {
      'qr_type': jsonString(getQrType()),
      'version': getVersion(),
      'coding_type': getCodingType(),
      'iban': jsonString(getIBAN()),
      'amount': getAmount() ?? '',
      'currency': jsonString(getCurrency()),
      'reference_type': jsonString(getReferenceType()),
      'reference': jsonString(getReference()),
      'additional_info': jsonString(getAdditionalInfo()),
      'creditor': _actors[actorCR].toJson(),
      'ultimate_creditor': _actors[actorUCR].toJson(),
      'ultimate_debitor': _actors[actorUDR].toJson(),
      'bill_information': jsonString(getBillInfo()),
      'trailer': jsonString(getTrailer()),
      'due_date': getDueDate(),
      'alternative_schema': getAlternativeSchema(),
    };
  }
...
class Actor {
  ...
  Map<String, dynamic> toJson() {
    String jsonString(String? input) => RegExp.escape(input ?? '');
    return {
      'address_type': jsonString(addressType),
